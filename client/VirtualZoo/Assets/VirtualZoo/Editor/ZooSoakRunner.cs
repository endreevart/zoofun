...
{
    public static class ZooSoakRunner
    {
        ...
        public static void Run()
        {
            ...
            File.WriteAllText(
                ...
                "mode=performance\n" +
                "capture=false\n");
            Debug.Log("ZooSoakRunner starting performance soak, duration=" + _duration);
        ...
        static void OnPlayMode(PlayModeStateChange change)
        {
            if (change == PlayModeStateChange.EnteredPlayMode)
            {
                ...
                Debug.Log("ZooSoakRunner entered play mode. capturePerformed=false");
                File.AppendAllText(
                    Path.Combine(EvidenceDir(), "soak-started.txt"),
                    "enteredPlayUtc=" + DateTime.UtcNow.ToString("o", CultureInfo.InvariantCulture) + "\n");
            }
        ...
        static void OnUpdate()
        {
            if (!_playing)
            {
                ...
                {
                    Debug.LogError("ZooSoakRunner watchdog: play mode did not start within 120s.");
                    Finish(2);
                }
            ...
            if (_collector.IsComplete)
            {
                ...
                {
                    Debug.LogError(
                        "ZooSoakRunner invariant failed samples=" + report.SampleCount +

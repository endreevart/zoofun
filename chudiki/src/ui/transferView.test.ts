...
import { defaultMoveDestId, fullMoveSummary, transferSummary, transferTitle } from './transferView.ts';

assert.equal(transferTitle('Жучок'), 'Переместить Жучок');
assert.equal(transferSummary('Жучок', 'Сад 2'), 'Жучок переедет в «Сад 2»');
assert.equal(fullMoveSummary('Куболесье'), 'Зуфунята переедут в «Куболесье»');

...
import { fetchWorlds, formatRub, quotePack, rememberOwnedWorlds, startCheckout, type Quote, type WorldOffer } from '../game/commerce';
...
export function WorldPicker({ worlds, onOpen, onError }: Props) {
  ...
  const empty = construction.length === 0;
  ...
  const pay = async (sku: string) => {
    ...
    const result = await startCheckout(sku, quote?.promo_code || promo.trim() || undefined);
    if (result.ok) {

...
- Status: accepted
- Date: 2026-09-03
- Revised: 2026-09-22 (discoveries CRM)
...
2. Credits, packs, and payments stay on the same Postgres ledger. CRM may send consented parent mail, CRUD promocodes, and moderate «Открытия ЗУФАН» through the API. It is not a second cash register; SQLAdmin `/staff` remains the write console for credits and prices.
3. Ported from kid: shell, dashboard cards, funnel hub/detail, traffic, usage, parent and payment lists, discoveries («Открытия ЗУФАН», same draft queue as Kid «Открытия MIO»). Not ported: partners, blog, banners, push, mascot, money-flow bank cards — those products do not exist here.

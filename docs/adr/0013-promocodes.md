# ADR-0013: Promocodes on generation packs, construction worlds, and plaza toys

- Status: accepted
- Date: 2026-09-14
- Revised: 2026-09-20 (`plaza_toy_1`, named code PRIVET)
- Relates: D-016, D-023, D-032, ADR-0006, ADR-0007, ADR-0008

## Context

CRM needed a discount tool without a second ledger. Parents type a code wherever the shop shows a price: site `/pricing`, island pack shop, island construction copies, and plaza drawing-toys.
...
2. `pack_ids` is a subset of generation packs `pack_1` / `pack_5` / `pack_10` / `pack_15` / `pack_20`, construction SKUs `world_diy_garden` / `world_diy_meadow` / `world_diy_grove`, and `plaza_toy_1`. Empty means all of those.
...
7. Named return-mail code `PRIVET` is seeded on API startup: 25% on packs 5/10/15/20 and DIY worlds. It does not apply to `pack_1` or `plaza_toy_1`. CRM offer templates name that code. Operator may deactivate it; startup restores kind, value, and SKU list, not the active flag.

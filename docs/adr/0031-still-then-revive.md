...
- Status: accepted
- Date: 2026-09-19
- Revised: 2026-09-19 (leftover 3D still starts Tripo on the next drawing)
...
3. If leftover 3D credits remain, the hatch «В сад!» spends one and starts Tripo (the egg waits for the GLB). The first free stylize still starts the mesh as soon as the still is shown. When 3D remaining is 0, later drawings spend a still and stop at `mesh_status=deferred`. «В сад!» then opens PackSheet (`pack_1` / `pack_5`, 10–20 behind expand). 3D on those postcards can also start on **Оживить**.
4. Packs, operator grants, and plaza crystals still increment `quota_total`. Each extra 3D slot adds ten stills by the formula. A pack of 10 after the free Zufik is 10 revive credits and 110 stills if the starter ten were unused.
5. Delete restores neither ledger. ADR-0024 prices stay. ADR-0026 local paper exists only when `still_remaining` is 0.
6. A postcard without a mesh is a finished garden creature until revive (revises D-015 for this unpaid card only). A paid mesh job that is merely delayed still must not open a standee as the 3D result.
